...
bool Map::IsPlantable(glm::vec3 position, glm::vec3* tile_position) {
    int tile_x = floor((position.x + (tile_size / 2)) / tile_size);
    int tile_y = -(ceil(position.y - (tile_size / 2))) / tile_size; // Our array counts
    int tile = levelData[tile_y * width + tile_x];
    if (tile != 0) return false;
    tile_position->x = (tile_x * tile_size);
    tile_position->y = -(tile_y * tile_size);
    return true;
}
